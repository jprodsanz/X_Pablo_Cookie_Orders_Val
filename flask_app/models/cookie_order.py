...
class Cookie_order:
    ...
    def __init__(self,cookie_order):
        ...
        self.number_boxes = cookie_order["number_boxes"]
    ...
    def is_valid(cls, cookie_order):
        ...
        if len(cookie_order["name"]) <= 0 or len(cookie_order["cookie_type"]) <= 0 or len(cookie_order["number_boxes"]) <= 0:
            valid = False
        ...
        if int(cookie_order["number_boxes"]) <= 0:
            valid = False
    ...
    def create(cls, cookie_order):
        ...
        query = """
                INSERT into cookie_orders (name, cookie_type, number_boxes)
                VALUES (%(name)s, %(cookie_type)s, %(number_boxes)s);"""
